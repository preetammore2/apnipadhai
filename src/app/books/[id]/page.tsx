    ...
    <div className="py-12 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Link href="/books" className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-brand-600 mb-6 transition-colors">
          ...
          <span>Back to Publication Store</span>
        </Link>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-card space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-500">SKU: {book.sku}</span>
              </div>
              ...
              <p className="text-slate-600 text-sm leading-relaxed">{book.subtitle}</p>
              ...
              <div className="flex items-center gap-4 text-xs text-slate-500 border-y border-slate-100 py-3">
                <span className="flex items-center gap-1 font-bold text-navy-900">
                  <Star className="w-4 h-4 fill-amber-400 text-amber-400" /> {book.rating} ({book.reviewsCount} Ratings)
                </span>
                <span>•</span>
                <span>Author: <strong className="text-navy-900">{book.author}</strong></span>
                <span>•</span>
              ...
              <div className="space-y-4 pt-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200">
                    You Save ₹{book.originalPrice - book.price}
                  </span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4 border-t border-slate-100 text-xs text-slate-600">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>100% Original Authentic Publication</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-card space-y-6">
              <div>
                <h3 className="text-xl font-bold font-heading text-navy-900 mb-2">Book Description</h3>
                <p className="text-sm text-slate-600 leading-relaxed">{book.description}</p>
              </div>

              <div>
                <h3 className="text-lg font-bold font-heading text-navy-900 mb-3">Table of Contents Highlights</h3>
