...
export default function AdminFeedbackPage() {
  ...
  const update = (index: number, next: FeedbackItem) => {
    const copy = [...items];
    copy[index] = next;
    setItems(copy);
    setResult(null);
  ...
  async function save() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.success) {
        setResult({ ok: false, message: data.message || 'Could not save feedback' });
        return;
      }
      setItems(data.items ?? items);
      setResult({ ok: true, message: 'Feedback saved. The site will refresh within a minute.' });
    } catch {
      setResult({ ok: false, message: 'Could not save feedback' });
    } finally {
      setSaving(false);
    }
  }
      ...
      <header className="bg-navy-900 text-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black font-heading leading-none">Feedback</h1>
              <p className="text-xs text-white/60 mt-1">Student reviews shown on /feedback</p>
            </div>
          </div>
          <LogoutButton />
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        {!configured && !loading && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 mb-6 text-sm text-amber-900">
            ...
            on the server to manage feedback from here.
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
          <div className="flex-1">
            <p className="text-sm text-slate-500">
              Entries added here are shown on the public feedback page. Reorder with the arrows and save.
            </p>          </div>
          <button
            onClick={load}
            disabled={loading}
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8">
            {items.length === 0 && (
              <div className="border border-dashed border-slate-200 rounded-2xl py-10 text-center mb-4">
                ...
                <p className="text-xs font-bold text-slate-400">No feedback entries yet.</p>
              </div>
            )}
            ...
            <div className="space-y-4">
              {items.map((item, index) => (
                <div key={index} className="border border-slate-200 rounded-2xl p-4 relative">
                  ...
                  <div className="mt-3 grid sm:grid-cols-1 gap-3 pr-8">
                    <ImageUploader
                      value={item.photo}
                  ...
                  <textarea
                    value={item.message}
                    onChange={(e) => update(index, { ...item, message: e.target.value })}
                    rows={3}
                    placeholder="What the student said…"
                    className="input-field mt-3 resize-y pr-8"
                  />
                </div>
              ))}
            </div>
