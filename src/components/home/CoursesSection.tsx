...
import { Star, Clock, ArrowRight, CheckCircle2, Flame, Loader2 } from 'lucide-react';
...
export const CoursesSection: React.FC = () => {
  ...
  const { data: courses = [], isLoading } = useGetCoursesQuery(activeCategory);

  const tabOptions = [
    { id: 'all', label: 'All Live Batches' },
    { id: 'rajasthan-gk', label: 'Rajasthan GK' },
    { id: 'cet', label: 'Rajasthan CET' },
    { id: 'ssc-gd', label: 'SSC GD' },
    { id: 'ras', label: 'RAS Pre + Mains' },
    { id: 'science', label: 'Science Brahmastra' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          <div className="py-16 text-center">
            ...
            <p className="text-xs text-slate-500 mt-2">Loading Live Batches via RTK Query...</p>
          </div>
        ) : (
          /* Course Cards Grid */
              ...
              >
                  ...
                  <div className="relative h-52 w-full bg-slate-100 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
                      {course.isBestseller ? (
                        <span className="px-3 py-1 bg-yellow-400 text-navy-950 text-[10px] font-black uppercase rounded-full shadow-sm flex items-center gap-1">
                          <Flame className="w-3 h-3 fill-navy-950" /> BESTSELLER
                        </span>
                      ) : (
                    ...
                    <div className="absolute bottom-3 left-3 right-3 text-white flex items-center justify-between">
                      <span className="text-xs font-medium flex items-center gap-1 text-slate-200">
                        <Clock className="w-3.5 h-3.5 text-yellow-400" /> {course.durationMonths} Months Access
                      </span>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100 mb-4">
                      ...
                      <div className="text-xs min-w-0">
                        ...
                        <p className="text-[10px] text-slate-500 truncate">{course.instructor.experience}</p>
                      </div>
                    </div>
                ...
                <div className="px-6 pb-6 pt-4 border-t border-slate-100 flex items-center justify-between gap-4 bg-slate-50/50">
                  <div>
                    ...
                    <span className="text-[10px] font-black text-emerald-600">{course.discountPercentage}% OFF TODAY</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-4.5 py-2.5 bg-yellow-400 hover:bg-yellow-500 text-navy-950 font-black rounded-xl text-xs shadow-button-glow transition-all"
                    >
                      Enroll Now
                    </button>
                    <Link
