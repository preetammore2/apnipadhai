          ...
          <div className="relative w-24 h-32 rounded-xl overflow-hidden shadow-md shrink-0 border border-slate-300">
            {book.coverImage ? (
              ...
              <div className="w-full h-full flex items-center justify-center p-2">
                <span className="text-[10px] font-black text-slate-300 text-center leading-tight">{book.title}</span>
              </div>
            )}
          </div>
        ...
        {book.tableOfContents && book.tableOfContents.length > 0 && (
          <div className="space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-navy-900 flex items-center gap-2">
              ...
              <span>{book.pages ? t('Sample Chapter Table of Contents ({0} Total Pages)', book.pages) : t('Sample Chapter Table of Contents')}</span>
            </h4>
